...
export function useLockGridSize(
  ...
  const oldSize = React.useMemo(
    () => oldGridSize * oldCellSize,
    [oldGridSize, oldCellSize]
  )

  const limitReached = React.useCallback(
    (value: number) => {
      if (value < 1) {
        setGridSizeInput(oldGridSize)
        setCellSizeInput(oldCellSize)
        throw new Error("Cannot lower this value further")
      }
    },
    ...
    if (lockGridSize) {
      try {
        ...
        if (gridSizeInput > oldGridSize) {
          ;[newGridSize, newCellSize] = handleChange(
            newGridSize,
            newCellSize,
            "increase"
          )
          ...
          ;[newCellSize, newGridSize] = handleChange(
            newCellSize,
            newGridSize,
            "increase"
          )
        ...
        if (
          ...
          error.message !== "Cannot lower this value further"
        ) {
          throw error
        }
      }
